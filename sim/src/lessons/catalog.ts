/**
 * Lesson catalog — foundation F-01–F-10, beginner tracks (C/S/X-B),
 * wave 1 scenario-linked lessons, wave 2 intermediate (LESSON_SYSTEM_BRIEF
 * §4.2, §4.4).
 *
 * Lesson prereqs remain ADVISORY until full pillar chains ship and owner
 * approves hard-flip (brief §7.3).
 *
...
export const LESSON_CATALOG: LessonDef[] = [
  ...
  def(LESSON_X_B01),
  def(LESSON_X_B02),
  def(LESSON_X_B03, "short"),
  def(LESSON_X_B04, "short"),
  def(LESSON_X_I01),
  def(LESSON_X_I02),
  def(LESSON_X_I03),
  def(LESSON_X_I04),
